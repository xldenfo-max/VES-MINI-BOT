...
// Initialize InMemoryStore for contacts and messages
...
if (!fs.existsSync(__dirname + '/sessions/creds.json')) {
    ...
    const sessdata = config.SESSION_ID.replace(config.SESSION_NAME, '');
...
async function connectToWA() {
    ...
    conn.ev.on('connection.update', (update) => {
        ...
        if (connection === 'close') {
            ...
            let up = `> *VES ULTRA MINI BOT CONNECTED SUCCESSFUL✅*

🤖BOT NAME_ *VES ULTRA MINI*
🖥️BOT DEVELOPMENT_ *VES INC*
👨‍💻BOT OWNER_ *MR.isira induwara*
✒️BOT CMD EXMAPLE_ *.alive*

*YOUR BOT ACTIVE NOW ENJOY 🎭*\n\n✒️PREFIX: ${prefix}`;
            conn.sendMessage(conn.user.id, { image: { url: `https://files.catbox.moe/80d5w9.png` }, caption: up })
        }
    })
    ...
    conn.ev.on('messages.upsert', async(mek) => {
        ...
        const jawad = ['94772257877', '94740544995', '94772257877'];
